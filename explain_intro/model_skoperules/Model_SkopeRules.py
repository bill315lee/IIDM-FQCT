...
class Skoprules:
    ...
    def __init__(self, args, feature_names, batch_size):
    ...
    def explain(self,
        ...
        for i in tqdm(range(len(explained_data_all))):
            ...
            clf = SkopeRules(n_estimators=20,
                             random_state=0,
            ...
            if self.train_all_data:
                ...
                if explain_instance_label in attack_dict:
                    attack_data = np.array(attack_dict[explain_instance_label][-3:])
                    data = np.vstack((pass_data, attack_data, explain_instance.reshape(1, -1)))
                    label = np.array(list(np.zeros(len(pass_data))) + list(np.ones(len(attack_data))) + [1])
                else:
                    data = np.vstack((pass_data, explain_instance.reshape(1, -1)))
                    label = np.array(list(np.zeros(len(pass_data))) + [1])
