...
class Incremental_Prototype_Weights(object):
    class Prototype:
        # init
        def __init__(self, feature, label, idx, size, fea_weights, proto_weights):
            if isinstance(idx, list):
                ...
                self.fea_weights = fea_weights
                self.proto_weights = proto_weights
                self.label = label

        def update_proto(self, feature, label, idx, fea_weights, proto_weights):

            self.center = (self.center * self.size + feature) / (self.size + 1)
            self.fea_weights = (self.fea_weights * self.size + fea_weights) / (self.size + 1)

            if len(proto_weights) != len(self.proto_weights):
                proto_weights_new = np.zeros_like(proto_weights)
                proto_weights_new[:len(self.proto_weights), :len(self.proto_weights)] = self.proto_weights
                self.proto_weights = (proto_weights_new * self.size + proto_weights) / (self.size + 1)
            else:
                self.proto_weights = (self.proto_weights * self.size + proto_weights) / (self.size + 1)
            ...
            self.r = np.max(self.euclidian_dist(self.center, np.array(self.instances)))
    ...
    def assign(self, feature, label, index, fea_weights, proto_weights):

        if label not in self._dict:
            prototype = Incremental_Prototype_Weights.Prototype(feature, label, index, 1, fea_weights, proto_weights)
            ...
            if min_dist <= self.threshold:
                ...
                proto.update_proto(feature, label, index, fea_weights, proto_weights)
            else:
                prototype = Incremental_Prototype_Weights.Prototype(feature, label, index, 1, fea_weights, proto_weights)
    ...
    def merge(self, label):
        ...
        tag_index = []
        # while dist_paire[0][0] < 0.66 * self.threshold and len(dist_paire) != 1:
        ...
        # while dist_paire[0][0] < dist_paire[0][2][0] + dist_paire[0][2][1] and len(dist_paire) != 1:
            ...
            if dist_paire[0][1][0] in tag_index or dist_paire[0][1][1] in tag_index:
                ...
                proto = Incremental_Prototype_Weights.Prototype(center, label, list(np.array(data_idx_lst)[sort_idx]), size)
                ...
                # if item < 0:
                #     item = 0
                # proto.r = np.sqrt(item)

                proto.r = np.max(self.euclidian_dist(np.array(proto.instances), center))
