...
def plotMatrixPoint(normal_data, attack_data, mu, std):
    ...
    plt.bar(x, prec_on_prototypes, width=width, label='Precision', tick_label=[r"$\bf{Ours}$", 'DeepAID-f', 'ATON', 'SHAP'], color= color1)
    for i in range(len(x)):
        x[i] = x[i] + width
    plt.bar(x, jac_on_prototypes, width=width, label='Jaccard index', color= color2)

    plt.ylabel("Values", fontsize=25)
    plt.xticks(size=20, rotation='30')
    plt.yticks(np.arange(0.0,1.01,0.2), size=20)

    plt.title("(a) Evaluation of Feature Explanations", size=25, x=0.5,y=1.025)
...
# ----------------------------------------------------------------------
    ...
    cbar_normal.set_label('Number of Normal Traffic', font={'size':20})
    ...
    cbar_attack.set_label('Number of Attacks', font={'size': 20})

    ticks = [0, 0.2, 1]
    labels = ['Normal\nTraffic', 'Attacks','']
    axes4.set_yticks(ticks)
    axes4.set_yticklabels(labels)

    plt.yticks(size=20)
    plt.xticks(size=20)
    plt.xlabel('Feature value', size=20)
    # axes4.get_yaxis().set_visible(False)

    plt.title('(b) Evaluation of Rule Explanations', size=25, x=0.5,y=1.05)
    plt.subplots_adjust(wspace=0.2)

    plt.savefig('./figure/intro_figure', bbox_inches='tight', dpi=256)
