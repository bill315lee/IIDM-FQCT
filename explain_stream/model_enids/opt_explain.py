...
class OPT_Explain:
    ...
    def results_print(self, explain_instance, feature_importance, centers, rs, data_metric_array):

        feat_lst_results = []
        imp_fea_num_lst = []
        feat_lst = [0.99]
        for fea in feat_lst:
            imp_idx = np.where(feature_importance >= fea + self.eps)[0]
            if len(imp_idx) == 0:
                imp_idx = np.array([np.argmax(feature_importance)])

            imp_fea_num = len(imp_idx)
            imp_fea_num_lst.append(imp_fea_num)

            replace = np.zeros(len(feature_importance))
            replace[imp_idx] = 1
            remain = np.abs(1 - replace)

            data_metric_new = replace * data_metric_array + remain * explain_instance
            dist_matrix = MatrixEuclideanDistances(data_metric_new, centers)
            in_circle_array = np.zeros(len(dist_matrix))
            for j in range(len(dist_matrix)):
                cover_idx = np.where(dist_matrix[j] <= rs ** 2 + self.eps)[0]
                if len(cover_idx) == 0:
                    in_circle = 0
                else:
                    in_circle = 1
                in_circle_array[j] = in_circle
            feat_lst_results.append(np.mean(in_circle_array))
    ...
    def results_print_another(self, explain_instance, feature_importance, centers, rs, data_metric_array):
        ...
        for k in [1,3,5]:
            in_circle_lst_k = []
            in_circle_weight_lst_k = []

            imp_idx = np.argsort(-feature_importance)[:k]
            replace = np.zeros(len(feature_importance))
            replace[imp_idx] = 1
            remain = 1 - replace
            data_metric_new = replace * data_metric_array + remain * explain_instance
            ...
            in_circle_lst_k.append(np.mean(in_circle_array))
            ...
            for j in range(len(dist_matrix_weight)):
                ...
                if len(cover_idx) == 0:
                    ...
                    in_circle_weight = 1
                in_circle_weight_array[j] = in_circle_weight
            in_circle_weight_lst_k.append(np.mean(in_circle_weight_array))

            in_circle_lst.append(np.mean(in_circle_lst_k))
            in_circle_weight_lst.append(np.mean(in_circle_weight_lst_k))




        return [round(x,4) for x in in_circle_lst],  [round(x,4) for x in in_circle_weight_lst]
