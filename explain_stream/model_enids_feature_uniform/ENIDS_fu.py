...
from explain_stream.model_enids_feature_uniform.opt_explain_fu import OPT_Explain_fu
...
class ENIDS_fu:
    ...
    def explain(self, explain_data, explain_label, explain_idx, proto_path):
        ...
        epoch_lst = []
        opt_exp_fu = OPT_Explain_fu(epochs=self.epochs,
                                  lr=self.lr,
                                  alpha=self.alpha,
                                  eps=self.eps,
                                  t1=self.t1,
        ...
        if self.store_w:
            ...
            for i in tqdm(range(len(explain_data))):
                ...
                if len(cover_idx) != 0:
                    ...
                    explanation, w_fea, epoch, instance_dist = opt_exp_fu.opt_exp(explain_instance, explain_instance_label, proto_array, w_fea_init)
                    ...
                    self.attacks_proto.assign(explain_instance, explain_instance_label, 0, w_fea, np.zeros((len(center_array), len(center_array))))
            ...
            for i in tqdm(range(len(explain_data))):
                ...
                if len(cover_idx) != 0:
                    ...
                    explanation_idx_in_attacks.append(i)
                    w_fea_init = torch.rand(len(explain_instance)).type(torch.float).to(self.device)
                    explanation, w_fea, epoch, instance_dist = opt_exp_fu.opt_exp(explain_instance, explain_instance_label,
                                                                         proto_array, w_fea_init)
                    effect_idx = []
                    for j in range(len(instance_dist)):
                        if 0 in instance_dist[j]:
                            effect_idx.append(j)
